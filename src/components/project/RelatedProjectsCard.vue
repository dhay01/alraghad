...
<template>
  <div class="container min-h-screen mx-auto px-5 sm:px-10 py-20" :dir="isRTL ? 'rtl' : 'ltr'">
    <div v-if="relatedProjects.length > 0">
      <h3 class="text-3xl md:text-5xl py-10 text-black" :class="isRTL ? 'text-right' : 'text-left'">
        {{ t('projectCard.relatedProjects') }}
      </h3>
        ...
        >
          ...
          <div class="card-body w-full text-black flex flex-col">
            <h2 class="card-title text-lg font-bold">
              {{ getLocalized('title', project) }}
            </h2>
            <!-- Project details -->
            <div class="details grid grid-cols-2 gap-4 flex-grow">
              <div class="text-sm font-semibold">{{ t('projectCard.location') }}:</div>
              <div class="text-sm">{{ getLocalized('location', project) }}</div>

              <div class="text-sm font-semibold">{{ t('projectCard.client') }}:</div>
              <div class="text-sm">{{ getLocalized('client', project) }}</div>

              <div class="text-sm font-semibold">{{ t('projectCard.startingDate') }}:</div>
              <div class="text-sm">{{ project.date || t('projectCard.notAvailable') }}</div>
            </div>
